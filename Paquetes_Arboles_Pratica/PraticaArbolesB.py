...
class Paquete:
    def __init__(self,id,prioridad,peso,tamaño,destino,fecha_entrega):
        self.id = id
        self.prioridad = prioridad
        self.peso = peso                # 2 fecha entrega igual la segunda 2: priodad es el peso
        self.tamaño = tamaño          # 3 el tamaño como prioridad si todo es igual
        self.destino = destino
        self.fecha_entrega = fecha_entrega ## 1 : prioridad maxima 

                                    # si todo es igual se da prioridad por eorden de llegada 
    def __str__(self):
        """Representación en forma de cadena del paquete."""
        return (f"ID: {self.id}")
...
def esNuevaRaiz(paquete_nuevo, paquete_nodo_actual):
    if paquete_nuevo.prioridad < paquete_nodo_actual.prioridad:             # pregunta la prioridad para saber 
        return True                                                         # i este debe ser la raiz del arbol
    if paquete_nuevo.prioridad == paquete_nodo_actual.prioridad:
